...
use App\Helpers\CustomHttpClient;
...
class CustomApi
{
    ...
    public function __construct(?string $token = null, bool $async = false, ?HttpClientInterface $httpClientHandler = null, ?string $baseBotUrl = null)
    {
        ...
        $this->httpClientHandler = $httpClientHandler ?? new CustomHttpClient();

        $this->baseBotUrl = $baseBotUrl;
        $api = new Api($token, $async, $this->httpClientHandler, $baseBotUrl);
        $this->commandBus = new CommandBus($api);
    }
